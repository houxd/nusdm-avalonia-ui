...
namespace nusdm
{
	public class MainWindowViewModel : ViewModelBase
	{
		...
		public bool IsIdle
		{
			...
			{
				isIdle = value;
				Dispatcher.UIThread.InvokeAsync(() => OnPropertyChanged());
			}
		}
		...
		private void DownloadFiles(string baseUrl, string saveDir, Tmd tmd)
		{
			...
			Parallel.ForEach(contentIds, new ParallelOptions { MaxDegreeOfParallelism = SettingsProvider.Settings.MaxDegreeOfParallelism }, (content, state, i) =>
			{
				...
				try
				{
					string contentFilePath = Path.Combine(saveDir, cidStr + ".app");
					...
					else
					{
						...
						wc.DownloadFile(baseUrl + cidStr, Path.Combine(saveDir, cidStr + ".app"));
					}
				}
				...
				if (SettingsProvider.Settings.DownloadH3Files)
				{
					...
					try
					{
						...
						wc.DownloadFile(baseUrl + cidStr + ".h3", Path.Combine(saveDir, cidStr + ".h3"));
					}
					catch (WebException e)
					{
			...
			_ = Task.Run(() =>
			  {
				  ...
				  {
					  saveDir = Path.Combine(saveDir, title.TitleType, title.TitleId.Replace(" ", ""));
				  }
				  else
				  {
					  saveDir = Path.Combine(saveDir, title.TitleType, $"[{title.Region}] {title.NameSanitized}");
				  }
					  ...
					  using (WebClient wc = new WebClient())
					  {
						  ...
						  if (!(titleType.Contains("game", StringComparison.OrdinalIgnoreCase) || titleType.Contains("demo", StringComparison.OrdinalIgnoreCase) || titleType.Contains("dlc", StringComparison.OrdinalIgnoreCase)))
						  {
							  ...
							  try
							  {
								  ...
								  try { File.WriteAllBytes(Path.Combine(saveDir, "title.tik"), ticket.ExportTicketData()); }
								  catch (IOException e) { AddLogEntry(" + ERROR! Could not save title.tik!"); Debug.WriteLine(e.Message); throw; }
							  }
							  catch (WebException e)
							  {
				  ...
				  {
					  AddLogEntry(" + ABORTING DOWNLOAD OF THIS TITLE: " + ex.Message);
